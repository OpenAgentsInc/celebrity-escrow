(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.NostrEscrow = factory());
    ...
    class JacobianPoint {
        ...
        static fromAffine(p) {
            ...
            return new JacobianPoint(p.x, p.y, _1n$5);
        }
    ...
    JacobianPoint.BASE = new JacobianPoint(CURVE.Gx, CURVE.Gy, _1n$5);
    JacobianPoint.ZERO = new JacobianPoint(_0n$5, _1n$5, _0n$5);
    ...
    let Point$1 = class Point {
        ...
        multiplyAndAddUnsafe(Q, a, b) {
            ...
            const aP = a === _0n$5 || a === _1n$5 || this !== Point.BASE ? P.multiplyUnsafe(a) : P.multiply(a);
    ...
    Point$1.ZERO = new Point$1(_0n$5, _0n$5);
    function sliceDER(s) {
    ...
    function kmdToSig(kBytes, m, d, lowS = true) {
        ...
        let recovery = (q.x === sig.r ? 0 : 2) | Number(q.y & _1n$5);
    ...
    function schnorrChallengeFinalize(ch) {
        return mod$1(bytesToNumber$1(ch), CURVE.n);
    }
    class SchnorrSignature {
        ...
        toRawBytes() {
            return hexToBytes$2(this.toHex());
        }
    }
    ...
    class InternalSchnorrSignature {
        ...
        initNonce(d, t0h) {
            return numTo32b$1(d ^ bytesToNumber$1(t0h));
        }
        finalizeNonce(k0h) {
            const k0 = mod$1(bytesToNumber$1(k0h), CURVE.n);
            if (k0 === _0n$5)
        ...
        finalizeSig(R, k, e, d) {
            return new SchnorrSignature(R.x, mod$1(k + e * d, CURVE.n)).toRawBytes();
        }
        error() {
    ...
    function finalizeSchnorrVerify(r, P, s, e) {
        const R = Point$1.BASE.multiplyAndAddUnsafe(P, normalizePrivateKey(s), mod$1(-e, CURVE.n));
    ...
    async function schnorrVerify$1(signature, message, publicKey) {
        try {
            ...
            const e = schnorrChallengeFinalize(await utils$1.taggedHash(TAGS.challenge, numTo32b$1(r), P.toRawX(), m));
    ...
    function schnorrVerifySync(signature, message, publicKey) {
        try {
            const { r, s, m, P } = initSchnorrVerify(signature, message, publicKey);
            const e = schnorrChallengeFinalize(utils$1.taggedHashSync(TAGS.challenge, numTo32b$1(r), P.toRawX(), m));
            return finalizeSchnorrVerify(r, P, s, e);
        }
        catch (error) {
    ...
    const utils$1 = {
        ...
        hashToPrivateKey: (hash) => {
            ...
            const num = mod$1(bytesToNumber$1(hash), CURVE.n - _1n$5) + _1n$5;
            return numTo32b$1(num);
        },
        ...
        sha256: async (...messages) => {
            if (crypto$3.web) {
                const buffer = await crypto$3.web.subtle.digest('SHA-256', concatBytes$3(...messages));
...
zoo`.split('\n');
    ...
    // HMAC (RFC 2104)
    let HMAC$2 = class HMAC extends Hash$1 {
        constructor(hash, _key) {
            super();
            this.finished = false;
            this.destroyed = false;
    ...
    utils$1.hmacSha256Sync = (key, ...msgs) => hmac$2(sha256$1, key, utils$1.concatBytes(...msgs));
    ...
    class HDKey {
        ...
        get privateExtendedKey() {
            ...
            return base58check.encode(this.serialize(this.versions.private, concatBytes$2(new Uint8Array([0]), priv)));
        }
        ...
        serialize(version, key) {
            ...
            bytes$1(key, 33);
            return concatBytes$2(toU32(version), new Uint8Array([this.depth]), toU32(this.parentFingerprint), toU32(this.index), this.chainCode, key);
        }
    }
    ...
    utils$1.hmacSha256Sync = (key, ...msgs) => hmac$1(sha256$3, key, utils$1.concatBytes(...msgs));
    ...
    class NostrEscrow {
        ...
        async getContract(role, nsec, event_id) {
            ...
            const { type, data } = !is_escrow ? nip19_exports.decode(nsec) : { type: null, data: nsec };
            ...
            const { shared_secret, plain, plain_reply, contract_hash } = await this.decryptAs(role, priv, maker_pub, taker_pub, sub, taker_reply?.content);
            const subcontract_serial = JSON.parse(plain);
            const [ver, escrow_pub, maker_sats, taker_sats, escrow_sats, contract_text, maker_sig,] = subcontract_serial;
            const confirm_hash = bytesToHex$1(sha256(utf8Encoder.encode(plain)));
            if (contract_hash) {
        ...
        async getHashFromEvent(priv, pub, sub) {
            ...
            const contract_hash = await nip04_exports.decrypt(priv, pub, encrypted_hash);
            return contract_hash;
        }
        async getContractSecretFromEvent(nsec, pub, sub) {
            const { type, data } = nip19_exports.decode(nsec);
            assert(type == "nsec", "need nsec");
            return this.getContractSharedSecret(data, pub, await this.getHashFromEvent(data, pub, sub));
        }
        async getContractSharedSecret(priv, pub, contract_hash) {
            const tweaked_pub = this.tweakPub(pub, contract_hash);
            const shared_secret = base64.encode(secp256k1.getSharedSecret(priv, "02" + tweaked_pub).slice(1, 33));
            return shared_secret;
        }
        async decryptAs(role, priv, maker_pub, taker_pub, sub, taker_reply) {
            ...
            async function decryptWith(tweaked_pub) {
                shared_secret = base64.encode(secp256k1.getSharedSecret(priv, "02" + tweaked_pub).slice(1, 33));
                plain = await nip04_exports.decrypt(priv, tweaked_pub, content);
                if (taker_reply)
                    plain_reply = await nip04_exports.decrypt(priv, tweaked_pub, taker_reply);
            }
        }
        async decryptWithSharedSecret(shared_secret, content) {
            if (!content)
                return "";
            const bytes_key = base64.decode(shared_secret);
            const cryptoKey = await crypto.subtle.importKey("raw", bytes_key, { name: "AES-CBC" }, false, ["decrypt"]);
        ...
        async createAcceptEvent(params) {
            ...
            const ev = {
                ...
                content: await nip04_exports.encrypt(taker_priv, tweaked_pub, JSON.stringify([0, params.taker_sig])),
            };
        ...
        async createContractEvent(params) {
            ...
            const subcontract_hash = bytesToHex$1(sha256(utf8Encoder.encode(subcontract_serial)));
            const tweaked_pub = this.tweakPub(params.taker_pub, subcontract_hash);
            const encrypted_hash = await nip04_exports.encrypt(maker_priv, params.taker_pub, subcontract_hash);
            const ev = {
                kind: 3333,
                tags: [
                    ["p", params.taker_pub],
                ...
                content: await nip04_exports.encrypt(maker_priv, tweaked_pub, subcontract_serial),
            };
        ...
        tweakAddPriv(priv, hex) {
            if (secp256k1.ProjectivePoint.fromPrivateKey(priv).toHex().startsWith("03")) {
                ...
                priv = bytesToHex$1(numberToBytesBE(pinv, 32));
            }
            const hash_bn = BigInt("0x" + hex);
            ...
            if (pt.toHex().startsWith("03")) {
                ...
                return bytesToHex$1(numberToBytesBE(inum, 32));
            }
            return bytesToHex$1(numberToBytesBE(pnum, 32));
        }
